...
class NAKOSource(Source):
    ...
    def _image_nii_w(id_, _root):
        file,  = Path(_root).glob(f'{id_}W*')
        return nibabel.load(file)

    def image_w(_image_nii_w):
        return _image_nii_w.get_fdata().astype(np.float32)

    def _image_nii_in(id_, _root):
        file,  = Path(_root).glob(f'{id_}in*')
        return nibabel.load(file)

    def image_in(_image_nii_in):
        return _image_nii_in.get_fdata().astype(np.float32)

    def affine(_image_nii_w):
        return _image_nii_w.affine
...
class NAKODataset(Dataset):
    def __init__(
        ...
        preprocessing = Chain(
            Transform(__inherit__=True, body_voxels=lambda image_w: np.argwhere(get_body_mask(image_w, BODY_THRESHOLD_MRI)))
        )

        pipeline = source >> preprocessing >> CacheToDisk.simple('image', 'body_voxels', root=cache_dir)

        self.ids = source.train_ids
        self.load_example = pipeline._compile(['image_w', 'image_in', 'body_voxels'])
...
def sample_views(
    ...
    patch_1, roi_voxels_1 = sample_view(image_w, roi_voxels, anchor_voxel, patch_size)
    patch_2, roi_voxels_2 = sample_view(image_in, roi_voxels, anchor_voxel, patch_size)
