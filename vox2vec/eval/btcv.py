...
DATA = [
    '/home/kats/storage/staff/eytankats/data/btcv/annotations/btcv_fold0_tr20.json',
    '/home/kats/storage/staff/eytankats/data/btcv/annotations/btcv_fold1_tr20.json',
    '/home/kats/storage/staff/eytankats/data/btcv/annotations/btcv_fold2_tr20.json'
]
...
class BTCVSource(Source):
    ...
    def train_ids(_root, _split):
        train_files = read_json_data_file(DATA[_split], '', 'training')
        return sorted({
            Path(file['image']).name[len('img'):-len('.nii.gz')]
            for file in train_files
        })

    @meta
    def test_ids(_root, _split):
        test_files = read_json_data_file(DATA[_split], '', 'validation')
        return sorted({
            Path(file['image']).name[len('img'):-len('.nii.gz')]
...
class BTCV(pl.LightningDataModule):
    ...
    def __init__(
        ...
        source = BTCVSource(root=root, split=split)

        train_val_ids = source.train_ids
        test_ids = source.test_ids
        train_ids, val_ids = train_test_split(train_val_ids, test_size=val_size, train_size=train_size, random_state=random_state)
        ...
        preprocessing = Chain(
            ...
            Transform(__inherit__=True, body_mask=lambda image: get_body_mask(image, BODY_THRESHOLD_HU)),
