...
class AMOSMRISource(Source):
    ...
    def train_ids(_root, _split):
        return sorted({
            file.name[len('amos_'):-len('.nii.gz')]
            for file in Path(_root).glob('**/dataset_preprocessed_mri/imagesTr/amos*.nii.gz')
        })
    ...
    def val_ids(_root, _split):
        return sorted({
            file.name[len('amos_'):-len('.nii.gz')]
            for file in Path(_root).glob('**/dataset_preprocessed_mri/imagesVa/amos*.nii.gz')
        })[:5]
